...
const outputPath = path.join(rootDir, "games", "officetel-watch", "latest-transactions.json");
const sourceUrl = "https://rt.molit.go.kr/";
const apiUrl = "https://apis.data.go.kr/1613000/RTMSDataSvcOffiTrade/getRTMSDataSvcOffiTrade";
...
async function fetchTextWithRetry(url, attempt = 1) {
  const response = await fetch(url, {
    headers: {
      ...
      "User-Agent": "TapTapCho Officetel Watch Sync"
    }
  });
...
async function fetchTransactionsByRegion(serviceKey, region, dealYearMonth) {
  ...
  return items.map((itemXml) => {
    ...
    const officetelName = getAnyTagValue(itemXml, ["offiNm", "단지명", "오피스텔"]);
    ...
    const buildYear = parseBuildYear(getAnyTagValue(itemXml, ["buildYear", "건축년도"]));
    const dealingGbn = getAnyTagValue(itemXml, ["dealingGbn", "거래유형"]);
    const estateAgentSggNm = getAnyTagValue(itemXml, ["estateAgentSggNm", "중개사소재지"]);
    const buyerGbn = getAnyTagValue(itemXml, ["buyerGbn", "매수자"]);
    const sellerGbn = getAnyTagValue(itemXml, ["slerGbn", "매도자"]);
    const cdealType = getAnyTagValue(itemXml, ["cdealType", "해제여부"]);
    const cdealDay = getAnyTagValue(itemXml, ["cdealDay", "해제사유발생일"]);
    const contractDate = toIsoDate(contractYear, contractMonth, contractDay);
    const canceledAt = normalizeCompactDate(cdealDay);
    const isCanceled = Boolean(cdealType || canceledAt);
    ...
    return {
      ...
      officetelName: officetelName || "오피스텔명 미확인",
      exclusiveAreaM2,
      floor,
      buildYear,
      contractDate,
      priceManwon,
      dealingGbn: dealingGbn || "거래유형 미확인",
      estateAgentSggNm,
      buyerGbn,
      sellerGbn,
      isCanceled,
...
async function main() {
  const serviceKey = normalizeServiceKey(
    process.env.OFFICETEL_API_SERVICE_KEY ||
      process.env.REAL_ESTATE_API_SERVICE_KEY ||
      process.env.DATA_GO_KR_API_SERVICE_KEY
  );
  const monthCount = Math.max(1, Number.parseInt(process.env.OFFICETEL_MONTH_COUNT || "3", 10) || 3);
  const nowIso = new Date().toISOString();

  if (!serviceKey) {
    throw new Error(
      "OFFICETEL_API_SERVICE_KEY is required. Use the decoded or raw data.go.kr service key as a GitHub secret."
    );
  }
  ...
  for (const dealYearMonth of months) {
    for (const region of regions) {
      ...
      for (const transaction of transactions) {
        if (!transaction.priceManwon || !transaction.contractDate || transaction.isCanceled) {
          continue;
        }
  ...
  if (!items.length) {
    throw new Error("No officetel transactions were collected. The output file was not updated.");
  }

  const payload = {
    snapshotMode: "live",
    updatedAt: nowIso,
    scope: "서울·경기 오피스텔 매매 자동 수집 스냅샷",
