...
function formatMeta(item) {
  const parts = [item.neighborhood, item.addressHint];

  if (item.buildYear) {
    parts.push(`${item.buildYear}년 준공`);
  }
...
function buildTransactionCards(items) {
  return items
    .map((item) => {
      const brokerText = item.estateAgentSggNm
        ? `중개 ${item.estateAgentSggNm}`
        : item.buildYear
          ? `${item.buildYear}년 준공`
          : "공개 자료 기준";
      ...
      return `
        <article class="transaction-card">
          ...
          <h3 class="transaction-title">${item.officetelName}</h3>
          <p class="transaction-meta">${formatMeta(item)}</p>
          ...
          <div class="transaction-foot">
            <span class="chip subtle">${item.dealingGbn || "거래유형 미확인"}</span>
            <span class="checked-at">${brokerText}</span>
          </div>
        </article>
      `;
    })
...
function renderStats(items) {
  ...
  statsGrid.innerHTML = `
    ...
    <article class="stat-card">
      ...
      <div class="stat-foot">${highest ? `${highest.district} · ${highest.officetelName}` : "-"}</div>
    </article>
    <article class="stat-card">
      <div class="stat-label">최근 계약일</div>
      <div class="stat-value">${latestContract ? formatDate(latestContract.contractDate) : "-"}</div>
      <div class="stat-foot">${latestContract ? latestContract.officetelName : "조건에 맞는 거래 없음"}</div>
    </article>
  `;
  ...
  try {
    ...
    const placeholder = `
      <article class="placeholder-card">
        ...
        <code>officetel-watch/latest-transactions.json</code> 파일을 확인해 주세요.
      </article>
    `;
