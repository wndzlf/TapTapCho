(() => {
  ...
  const RUNTIME_CLASS = "real-estate-watch-runtime";
  ...
  async function callAsyncBridge(method, args = [], fallback = false) {
    ...
    return new Promise((resolve, reject) => {
      ...
      console.warn(`[real-estate-watch/toss-runtime] ${method} failed`, error);
      return fallback;
    });
  }
  ...
  function getConstantBridgeValue(name, fallback) {
    ...
    try {
      ...
      console.warn(`[real-estate-watch/toss-runtime] ${name} failed`, error);
      return fallback;
    }
  }
  ...
  function subscribeToBridge(method, listener) {
    ...
    const unsubscribeError = onNativeEvent(`${method}/onError/${eventId}`, (payload) => {
      console.warn(`[real-estate-watch/toss-runtime] ${method} event failed`, reviveBridgeError(payload));
    });
  ...
  window.RealEstateWatchToss = toss;
    ...
    if (!document.getElementById(EXIT_MODAL_ID)) {
      ...
      modal.innerHTML = `
        ...
        <section class="runtime-sheet" role="dialog" aria-modal="true" aria-labelledby="runtime-exit-title">
          ...
          <h2 class="runtime-sheet-title" id="runtime-exit-title">서울경기실시간아파트를 종료할까요?</h2>
