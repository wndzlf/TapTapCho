...
토스가 게임 등급을 직접 발급해주는 구조는 아니다.  
외부에서 받은 게임 등급을 앱인토스 콘솔의 `게임 등급분류` 단계에 등록하는 방식이다.

등급을 확보하는 방법은 2가지다.

1. `게임물관리위원회(GRAC)` 에 직접 신청한다.
2. `App Store / Google Play / ONE store / Microsoft Store` 등 스토어의 `IARC 연령 등급`을 활용한다.

현재 `Orbit Survivor` 처럼 토스 중심으로 먼저 출시하는 경우에는 `GRAC 직접 신청`이 가장 현실적이다.

- 스토어 방식은 실제 스토어 출시 URL이 있어야 토스에서 등급 확인이 가능하다.
- 아직 스토어에 출시되지 않았다면 `GRAC 직접 신청 -> 등급분류증명서 등록` 흐름으로 가는 편이 안전하다.

`GRAC 직접 신청` 흐름:

1. [게임물관리위원회](https://www.grac.or.kr) 로그인
2. `등급분류신청 > 오픈마켓/등급분류 신청` 선택
3. 게임 정보 입력
4. 앱인토스용이면 `플랫폼: 모바일 게임`, `스토어명: 기타-앱인토스`, `서비스 방식: 인터넷 접속`으로 작성
5. 내용정보 작성
6. 심의용 자료 업로드
7. 수수료 납부 후 신청 완료
8. 심의 완료 후 `등급분류증명서` 발급
9. 토스 콘솔 `앱 등록 > 게임 등급분류` 단계에 해당 정보 등록
...
- 게임 진행 영상
- 게임물내용설명서
- 초상권 / 라이선스 관련 서류가 있으면 함께 첨부
- 사업자 신청이면 추가 서류가 필요할 수 있음

`스토어(IARC) 등급 활용` 흐름:

1. 스토어 개발자 계정에서 게임을 실제로 등록한다.
2. 스토어의 연령 등급 설문을 작성해 등급을 받는다.
3. 앱인토스 콘솔 `게임 등급분류` 단계에서 `자체등급분류 게임물 정보`를 추가 입력한다.
4. 스토어 URL 기준으로 토스가 등급을 확인한다.

스토어 등급 방식을 쓸 때 콘솔에 추가로 넣는 항목:
...
- 내용정보
- 대표자 인감 또는 사인 이미지
- 스토어 플레이 화면과 앱인토스 플레이 화면 이미지

운영 메모:

- 토스 FAQ 기준으로 게임은 등급 정보 없이 정식 출시할 수 없다.
- 토스 공개 가이드 기준으로 심의는 대략 영업일 기준 `14~15일` 정도를 예상한다.
- `청소년이용불가` 게임은 스토어 등급만으로 끝나지 않을 수 있으니 별도 심의를 다시 확인한다.
- Orbit Survivor 현재 상태에서는 `GRAC 직접 신청 -> 등급분류증명서 등록`을 기본 경로로 본다.

참고 문서:

- [게임 등급분류 신청 방법](https://toss.im/apps-in-toss/blog/game_rating_classification)
- [자체등급분류 게임물 정보, 간단하게 입력하기](https://toss.im/apps-in-toss/blog/self-rated_game_distribution)
- [앱 등록 가이드](https://developers-apps-in-toss.toss.im/prepare/checklist.html)
